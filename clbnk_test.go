...
const (
	TEST_DOC_COUNT       = 2
	TEST_DOC1_PAYER_ACC  = "40702810000000074935"
...
func TestImport(t *testing.T) {
	...
	doc2, ok := imp.Documents[1].(*PPDocument)
	if !ok {
		t.Fatal("document[1] must be of type PPDocument")
	}
	if doc2.PayerAccount != TEST_DOC1_PAYER_ACC {
		t.Fatalf("document[1] payer account, expected %s, got %s", TEST_DOC1_PAYER_ACC, doc2.PayerAccount)
	}
	if doc2.PayerInn != TEST_DOC1_PAYER_INN {
		t.Fatalf("document[1] payer inn, expected %s, got %s", TEST_DOC1_PAYER_INN, doc2.PayerInn)
	}
	if doc2.PayerName != TEST_DOC1_PAYER_NAME {
		t.Fatalf("document[1] payer name, expected %s, got %s", TEST_DOC1_PAYER_NAME, doc2.PayerName)
	}
	if doc2.ReceiverInn != TEST_DOC1_REC_INN {
		t.Fatalf("document[1] receiver inn, expected %s, got %s", TEST_DOC1_REC_INN, doc2.ReceiverInn)
	}
	if doc2.ReceiverAccount != TEST_DOC1_REC_ACC {
		t.Fatalf("document[1] receiver account, expected %s, got %s", TEST_DOC1_REC_ACC, doc2.ReceiverAccount)
	}
	if doc2.Sum != TEST_DOC1_SUM {
		t.Fatalf("document[1] sum, expected %f, got %f", TEST_DOC1_SUM, doc.Sum)
	}
}
